---
title: "37 · 检查点（Checkpoints）：随时能倒带的安全网"
description: "都说检查点是你的「后悔药」，有它兜底就能放开手脚让 Claude 大干一场——说句实话，把它当 git 用，迟早翻车。"
---

> 📚 **系列导航**：上一篇 [36 斜杠 / 命令](/blog/claude-code/36-slash-commands/) 把 `/` 开头那一长串命令理顺了，`/rewind` 也在里头露过脸。这一篇专门把它拆开讲——**检查点（Checkpoint）是 Claude Code 自动给你存的「编辑前快照」，让你随时倒带回某个状态**。它怎么自动记、怎么回退、能回滚什么不能回滚什么、跟 git 到底怎么分工，一篇说透。
...
![检查点时间轴：每条提示前自动存档；改坏了 /rewind 倒带回任意检查点（代码 + 对话一起回）](https://img.yuancong.ai/guide/claude-code/37-checkpoint-rewind@2x.png)

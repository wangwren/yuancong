---
title: "05 · 接入第三方 / 国产模型"
description: "兄弟们，先说一句可能挨骂的话：大多数人根本不需要折腾第三方模型。"
---

> 用 DeepSeek 等国产 / 第三方模型驱动 Claude Code，把账单砍下来

> 📚 **系列导航**：上一篇 [04 · API 配置](/blog/claude-code/04-api-config/) 讲了怎么用官方 API Key 接通 Claude Code。这一篇换个思路——**把后面那个「大脑」整个换掉**，用 DeepSeek 之类的国产模型来省钱。
...
前提是你已经装好了 Claude Code（没装的回去看 [02 · 安装与使用](/blog/claude-code/02-install/)）。然后：
...
![一个请求从 Claude Code 经环境变量改写后寄到 DeepSeek 又原路返回的时序图](https://img.yuancong.ai/guide/claude-code/05-request-flow-deepseek@2x.png)
...
下一篇 **[06 · Coding Plan：订阅套餐与计费](/blog/claude-code/06-coding-plan/)**——既然这篇一直在说「省钱」，那就把账算到底：**Claude 的订阅套餐（Pro / Max）和按量付费的 API，到底哪个划算？** 留个问题给你先想想：如果你每天都重度用 Claude Code，是该买订阅，还是接第三方 API 更省？看完下一篇你就有答案了。

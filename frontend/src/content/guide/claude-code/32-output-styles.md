---
title: "32 · 输出样式（Output Styles）：换一档「节目」，不换主持人"
description: "都说「想让 Claude 听话，就往 CLAUDE.md 里堆」——堆约定、堆规矩、堆它该记住的一切。"
---

> 📚 **系列导航**：上一篇 [31 settings.json：用户级 / 项目级配置](/blog/claude-code/31-settings-json/) 把「配置写哪个文件、谁压谁」捋清楚了。这一篇接着讲一个就住在那套配置里的开关——**输出样式（output styles）**。它管的不是「Claude 知道什么」，而是「Claude 怎么回应你」。一行设置，就能让它从默认的「闷头干活的工程师」，切成边讲解边干、甚至根本不写代码的另一副样子。
...
> 这里有个容易混的点，官方专门点了：Proactive 提供的是比[自动模式](/blog/claude-code/20-permissions/)更强的「自主执行**指导**」，但它**无需更改你的权限模式**就能用——所以**工具运行前你照样会看到权限提示**。权限模式（第 20 篇讲的「实习生动手前问不问你」）管的是「要不要拦着问你」，Proactive 管的是「它的行事风格多激进」，两码事，别当成一个。
...
> 运行 `/config` 并选择**输出样式**从菜单中选择一种样式。你的选择会保存到[本地项目级别](/blog/claude-code/31-settings-json/)的 `.claude/settings.local.json`。
...
| **托管策略级** | [托管设置目录](/blog/claude-code/31-settings-json/)内的 `.claude/output-styles` | 由组织统一下发 |
...
![Output Styles 三种样式的系统提示组装：内置 / 自定义保留工程指令 / 自定义抽掉工程指令；改样式后必须 /clear 才生效](https://img.yuancong.ai/guide/claude-code/32-style-assembly@2x.png)

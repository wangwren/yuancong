...
description: "先讲个坑——我装第一个 MCP server 那会儿，对着终端的报错折腾了快一个小时。"
---

> 📚 **系列导航**：上一篇 [21 安全与风险边界](/blog/claude-code/21-security/) 帮你想清楚「什么时候该信任 AI 碰你的代码和系统」。这一篇换个方向——**给 Claude 接上外部世界**。它默认只能摸你本地的文件和命令行，碰不到你的数据库、Jira、Figma。MCP，就是那个让它一次接上一堆外部服务的统一对接口。
...
![Claude Code 通过 MCP 这一层统一对接外部服务](https://img.yuancong.ai/guide/claude-code/22-mcp-hub@2x.png)

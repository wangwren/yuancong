---
title: "38 · 插件参考手册：把自己那套配置，打成一个能发出去的包"
description: "给一个插件跑一下 claude plugin details，输出里常有一行数字值得留意：这个插件每个会话常驻占 ~180 token…"
---

> 📚 **系列导航**：上一篇 [37 检查点（Checkpoints）](/blog/claude-code/37-checkpoints/) 教你怎么给会话「存档读档」，改坏了一键回到上一个安全点。这一篇换个层面——**第 24 篇教你「用别人的插件」，这一篇教你「造自己的插件」**：插件的目录到底怎么摆、`plugin.json` 每个字段管什么、能打包哪些组件、依赖怎么写、怎么建市场把它发给团队。这是插件的「参考手册」深度版。

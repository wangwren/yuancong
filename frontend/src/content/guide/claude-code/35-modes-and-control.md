---
title: "35 · 控制与模式：开会话时手里那块「调音台」"
description: "兄弟们，咱们这一路下来，命令也会敲了、CLAUDE.md 也会写了、权限也配过了，按说该挺顺了。"
---

> 📚 **系列导航**：上一篇 [34 CLI 参考手册：命令与全部标志](/blog/claude-code/34-cli-reference/) 帮你把「启动 Claude 之前」能拨的开关查全了。这一篇换到「会话已经开起来」之后——你跟 Claude 一来一回的过程里，手里其实攥着一整块「控制台」：怎么按一下就让它换工作模式、怎么让它先出方案别急着动手、怎么花钱换更快的响应、怎么用 Vim 手势编辑你的输入。**[第 20 篇](/blog/claude-code/20-permissions/)讲过权限那根缰绳，这一篇讲你手上的全套操纵杆。**
...
说白了，**Claude Code 的交互界面，远不止「输入框 + 回车」这么简单**。它底下藏着一整套「会话内的控制手段」：一个键切换它的工作模式、一个键把它从跑偏里拽回来、一个命令让它先谋后动、还有花钱买速度的开关。这些东西[第 14 篇](/blog/claude-code/14-interface-and-shortcuts/)露过几个快捷键的脸，[第 20 篇](/blog/claude-code/20-permissions/)讲透了权限那一支，但「会话里到底能怎么操纵它」一直没有集中铺开过。
...
- `Shift+Tab` 循环权限模式的全貌：默认三档之外的 `auto` / `bypassPermissions` / `dontAsk` 怎么进循环、谁排在谁前面（承接[第 20 篇](/blog/claude-code/20-permissions/)，不重讲权限规则）
...
- **管「它的自主权」**——也就是权限模式：`Shift+Tab` 一键循环 `default` / `acceptEdits` / `plan`，决定它动手前问不问你（[第 20 篇](/blog/claude-code/20-permissions/)主讲）。
...
第二种就是「倒车」：**把整段对话退回到之前某个状态**。这块属于检查点（checkpoint）的范畴，[第 37 篇](/blog/claude-code/37-checkpoints/)专门讲，这里你先记住一个动作：**输入框空着的时候双击 `Esc`，能掀开「回到上一个存档点」的菜单**。返工的时候常用——感觉「最近这几轮把事情带沟里了」，双 `Esc` 退回到沟之前那个干净点，重新开口。
...
权限模式那张「从严到松」的光谱，[第 20 篇](/blog/claude-code/20-permissions/)已经讲透了——`default` 步步问、`acceptEdits` 改码免问、`plan` 只看不动、`auto` 有分类器兜底、`bypassPermissions` 完全裸奔、`dontAsk` 只认预批准。**这一节不重讲它们各是啥（忘了回去翻[第 20 篇](/blog/claude-code/20-permissions/)），只补一件那篇没细抠的事：`Shift+Tab` 这个循环，到底怎么转。**
...
推荐的实操习惯，跟[第 20 篇](/blog/claude-code/20-permissions/)说的一致——**绝大多数时候只在默认三档里 `Shift+Tab` 手动切**：接陌生项目先切 `plan` 让它通读出方案，信得过方向了切 `acceptEdits` 放它改，改完想严一点再切回 `default`。`bypassPermissions` 只在隔离容器里用启动参数开，别挂进日常循环——这道红线[第 20 篇](/blog/claude-code/20-permissions/)画过，不重复了。
...
这一篇把「会话已经开起来之后，你手里那块控制台」铺开讲了一遍。[第 20 篇](/blog/claude-code/20-permissions/)给你的是权限那根缰绳，这一篇给你的是全套操纵杆。
...
| 退回对话上一个点 | `Esc` `Esc`（空框时） | 打开回退菜单，细节见[第 37 篇](/blog/claude-code/37-checkpoints/) |

...
description: "都说 CLAUDE.md 写得越详细越好，但说句实话，最没用的 CLAUDE.md，恰恰是那种写了三百行、Claude 一条都没听的。"
---

> 📚 **系列导航**：上一篇 [17 图片与多模态](/blog/claude-code/17-images-multimodal/) 让你学会把截图、报错图直接喂给 Claude。这一篇换个方向——把项目的「规矩」一次性写进它的记忆，让它每次开工都自动遵守，不用你天天复读。
...
> ℹ️ 本篇只聚焦 CLAUDE.md 这一个文件怎么写好、怎么维护。`/init` 一键生成的玩法在 [12「项目初始化」](/blog/claude-code/12-project-init/) 讲过了，更广的自动记忆（auto-memory）机制留给 [25「记忆系统」](/blog/claude-code/25-memory/) 专门聊。
...
![CLAUDE.md 三层级加载：拼接不覆盖](https://img.yuancong.ai/guide/claude-code/18-claude-md-layers@2x.png)
...
> ℹ️ 一个版本差异提醒：早期 Claude Code 里，在输入框用 `#` 开头打一句话能快速追加记忆。新版本里这套交互已经变了——**以官方现在的做法为准：要么直接让 Claude「加进 CLAUDE.md」，要么用 `/memory` 自己编辑**。你敲一句「记住 xxx」，Claude 默认多半存进它自己的自动记忆（auto-memory，那是 [25「记忆系统」](/blog/claude-code/25-memory/) 的话题）；想明确进 CLAUDE.md，就把「加进 CLAUDE.md」这句话说全。

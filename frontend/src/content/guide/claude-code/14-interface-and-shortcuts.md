...
description: "「欸，你这 Esc 按一下和按两下不一样？」"
---

> 📚 **系列导航**：上一篇 [13 项目结构](/blog/claude-code/13-project-structure/) 带你看清 Claude Code 在你项目里都摆了哪些文件。这一篇换个视角——回到那个终端窗口本身，搞懂界面每一块是干嘛的，再把几个能让你「快一倍」的键盘动作焊进肌肉记忆。
...
![交互界面分区与高频快捷键](https://img.yuancong.ai/guide/claude-code/14-interface-keys@2x.png)
...
在输入框开头打 `!`，后面跟一条命令，就进入 **Shell 模式（Bash 模式）**：这条命令**直接在你的终端跑**，不用 Claude 批准、也不用它先「理解」你的话再替你跑，跑完的输出会被加进对话上下文里。
...
- **默认（v2.1.186 起）：自动回你一句。** 比如 `! npm test` 跑完，它会直接跟一句解释哪儿失败了，不用你再问。省事，但**这次回复按一条普通提示词计费**——所以默认情况下 `!` 省的是「描述需求 + 等批准」那一来一回，不是 token。
- **在 `settings.json` 里设 `respondToBashCommands: false`：只记录、不回话。** 回到 v2.1.186 之前的老行为——输出只进上下文，当下不发起任何模型调用，「省 token」这个卖点就回来了。严格说也不是零成本：这段输出会随你下一条消息按输入 token 计费，通常小到可以忽略；但要是 `!` 跑了个刷屏的长日志，这个尾巴就不小了。
...
回车。**预期**：终端**直接**打印出 `hello-shortcuts`（没有经过「征求批准」那一步），命令和输出被记进对话，然后 Claude 会针对这个输出简单回一句（默认设置下）——这就是 Shell 模式的完整流程。
...
下一篇 [**15「怎么提问和给指令」**](/blog/claude-code/15-prompting/)——手放对地方了，接下来就是「嘴」的功夫了。同样一个需求，会问的人三句话搞定，不会问的人来回扯五轮还跑偏。下一篇咱们就聊：**到底怎么跟 Claude 说话，它才听得准、干得对？**

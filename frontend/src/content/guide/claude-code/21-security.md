...
description: "安全研究者反复演示过这样一类攻击，而且对 Claude Code、Gemini CLI、GitHub Copilot 这些主流编程助手都验证有效…"
---

> 📚 **系列导航**：上一篇 [20 权限配置](/blog/claude-code/20-permissions/) 教你怎么写 `allow` / `ask` / `deny`、怎么用 `Shift+Tab` 切模式——那是「缰绳怎么攥」。这一篇往上一层：缰绳攥在手里了，可**到底该不该放手让 AI 碰你的代码和系统？真正的高危区在哪？提示注入（prompt injection）、敏感数据泄露这些坑长什么样、怎么防？** 讲的是「判断力」，不是「配置项」。
...
![Claude Code 安全的分层防护：从权限规则到独立虚拟机](https://img.yuancong.ai/guide/claude-code/21-defense-layers@2x.png)

---
title: "46 · 开发配置：把 Claude 干活的「工作环境」调顺"
description: "这里先摆一个很多人都会撞上的场景。"
---

> 📚 **系列导航**：上一篇 [45 Agent SDK](/blog/claude-code/45-agent-sdk/) 教你把 Claude Code 的本事拆出来、塞进自己写的程序里。这一篇收回到你每天敲命令的那个终端——**Claude 跑在哪、能碰到哪、走不走代理、长什么样、用哪个模型**，这五件「工作环境」的事，是时候按自己的需要调一遍了。这一篇就把开发配置里最常用的几块讲透。
...
你用到现在，大概率被一个矛盾夹过：**想让 Claude 跑得顺，又怕它跑得太野。** 权限模式（[第 20 篇](/blog/claude-code/20-permissions/)讲过）只有两头——要么每条命令都停下来问你（烦），要么开自动模式放它撒欢（怕）。沙箱给的是中间那条路。
...
默认边界很克制——**沙箱里的命令只能写当前工作目录**，命令第一次要碰新网络域时，Claude Code 才停下来问你批不批。在面板里选的模式，会写进项目的 `.claude/settings.local.json`（只管当前项目、不进 git）。**想让所有项目都默认开沙箱**，就去用户级的 `~/.claude/settings.json` 把 `sandbox.enabled` 设成 `true`（settings.json 的层级规则见[第 31 篇](/blog/claude-code/31-settings-json/)）。
...
| **Claude Code on the web**（[第 11 篇](/blog/claude-code/11-web-and-cloud/)） | 整个操作系统，Anthropic 托管 | 否 | 不想自己搭环境、没本地环境也能用 |

把这几种方式按「圈得有多严」从左到右排一排，你心里就有杆秤了——**越往右隔离越强、配起来越费劲，按你对代码的信任程度往右挪**：

![Claude Code 五级隔离梯度：不开 → 自带沙箱 → devcontainer → 容器/VM → 云端托管](https://img.yuancong.ai/guide/claude-code/46-isolation-levels@2x.png)
...
**国内用户重点看这条**：`api.anthropic.com`、`claude.ai` 这些在国内直连大概率不通，**你需要「魔法上网」**，或者走[第 04 篇](/blog/claude-code/04-api-config/) / [第 05 篇](/blog/claude-code/05-third-party-models/)讲的第三方中转、国产模型路线。这一节的代理配置，本质也能拿来指向你的中转网关。要是你用的是 Amazon Bedrock、Google Vertex 这类第三方提供商，模型流量走的是它们的地址，就不必放行上面这几个 Anthropic 域名了。
...
在 iTerm2 上把这个配好之后，**挂着长任务去倒杯咖啡，回来一看进度，效率高了不止一点**——不然就是干等着盯屏幕，生怕错过那个权限提示。想要更花哨的（比如播个自定义音效），还能配「通知钩子」，那属于[第 33 篇](/blog/claude-code/33-hooks/) Hook 的范畴，这里知道有这条路就行。
...
顺带提一句：macOS 上不少 `Option+` 开头的快捷键（比如 `Option+P` 换模型）默认不生效，得先在终端里把 Option 配成 Meta 键（iTerm2 在「设置 → 配置文件 → 键」里把左 / 右 Option 设成「Esc+」）。这块[第 35 篇](/blog/claude-code/35-modes-and-control/)详细讲过，这里不重复。还有 Vim 党：`/config` 里把编辑器模式设成 `vim`，输入框就能用 Vim 手势编辑了。
...
最后一块，**模型配置**——直接关系到「贵不贵、聪不聪明」。[第 04 篇](/blog/claude-code/04-api-config/)、[第 06 篇](/blog/claude-code/06-coding-plan/)讲过怎么接模型、怎么计费，这一节只讲「怎么按任务挑、怎么换、怎么省」。
...
记一个实用判断：**临时换一下用 `/model`，想固定成某个项目 / 某个人的默认就写 `settings.json` 的 `model` 字段**（放哪一层看[第 31 篇](/blog/claude-code/31-settings-json/)）。要注意项目设置和企业托管设置优先级更高，会盖过你个人的选择——所以要是你发现「我明明设了 Opus，启动却是别的」，多半是被项目里的 `model` 字段压住了。

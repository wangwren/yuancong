---
title: "37 · 常见问题排查：装不上、登不了、不肯改文件，挨个拆"
description: "「兄弟，我 npm install 完了，敲 codex 说 command not found，咋办？」"
---

> 📚 **系列导航**：上一篇〔[36 最佳实践](/blog/codex/36-best-practices/) 〕讲的是「怎么用对、怎么用顺」，把好习惯沉淀成肌肉记忆。这一篇反过来——**专治各种用不顺**：装不上、登不进、它死活不肯改你文件、聊着聊着变笨……把高频坑挨个拆给你看。下一篇〔[38 术语表](/blog/codex/38-glossary/) 〕是整个 Codex 篇的收尾词典，遇到生词回去查。
...
> 平台差异：Windows 用户的「装不上」往往是另一码事（缺 WSL、路径问题等），那一类坑在〔[33 Windows 使用要点](/blog/codex/33-windows/) 〕单独讲，这里不展开。
...
- **临时放开一次**，用命令行参数。沙箱用 `--sandbox`（简写 `-s`），审批用 `--ask-for-approval`（简写 `-a`）。想让它在当前项目里放手改、出项目才喊你，就用「工作区可写 + 按需审批」这套日常黄金组合：
  ...
  更多取值和组合在〔[15 权限、沙箱与审批](/blog/codex/15-permissions/) 〕讲透了，这里不重复。
...
MCP 是个 USB 接口的概念在〔[20 MCP](/blog/codex/20-mcp/) 〕讲过，这里只管排错。我自己配第一个 MCP 时卡了半小时，最后发现是漏给了一个环境变量——**单独跑一次那个服务，比对着 Codex 干瞪眼有效十倍。**
...
- **订阅被限流**，要么等额度刷新，要么升套餐。日常省额度的关键是别无脑顶格——简单活儿用 `gpt-5.4-mini`、低推理强度，把旗舰 + 高推理留给真正的硬骨头。这套在〔[30 怎么选模型](/blog/codex/30-models/) 〕讲透了。
...
**解法**：原生 Windows 上想要最接近 Linux 的体验，官方建议用 WSL（Windows Subsystem for Linux）。**Windows 的安装、路径、WSL 配置这些专属坑，集中在〔[33 Windows 使用要点](/blog/codex/33-windows/) 〕讲**，遇到带 Windows 味儿的问题直接去那篇查，别在这儿瞎试。
...
**解法**：`1385` 找 IT 放行登录权限；`1223` 别去折腾 UAC 和杀软，看到「libpng 警告 + 找不到模块弹窗」这个组合，直接 `npm uninstall -g @openai/codex` 再 `npm install -g @openai/codex` 重装覆盖；都解不了先切 `unelevated` 顶着，完整排查链在〔[33 Windows 使用要点](/blog/codex/33-windows/) 〕第 05 节。
...
下一篇〔[38 术语表](/blog/codex/38-glossary/) 〕是整个 Codex 篇的收尾——把这一路出现过的术语（沙箱、审批、推理强度、MCP、子代理、`codex exec`……）做成一份按字母 / 拼音可查的词典，哪个词记混了回去翻一翻。临走留个小思考：这一篇所有解法里，有几条其实都指向同一个习惯——**动手前先把「退路」和「权限」想清楚**。你能数出是哪几条吗？

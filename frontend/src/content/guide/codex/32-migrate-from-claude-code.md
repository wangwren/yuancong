---
title: "32 · 从 Claude Code 迁移：旧地图换个工具，照样能找到家"
description: "先还原一段真实对话。上个月一个一直用 Claude Code 的朋友装好 Codex，开口就是一连串问号："
---

> 📚 **系列导航**：上一篇〔[31 进阶技巧与提速](/blog/codex/31-speed/) 〕讲怎么把整条工作流跑得更快、更省、更少返工。这一篇换个角度——专门写给**从 Claude Code 转过来的你**：你脑子里那套用了大半年的心智模型，哪些能直接搬、哪些得改个名、哪些是 Codex 独有的新东西。下一篇〔[33 Windows 使用要点](/blog/codex/33-windows/) 〕再聊 Windows 上那些专属的坑。
...
至于模型对照，一句话带过：你在 Claude Code 那边按「Opus 啃硬活、Sonnet 跑日常、Haiku 跑杂活」选型号，到 Codex 这边换成「`gpt-5.5` 啃硬活、`gpt-5.4-mini` 跑杂活」，选型逻辑（拿任务难度匹配算力）一模一样，详见〔[30 怎么选模型](/blog/codex/30-models/) 〕。注意别把 `gpt-5.4` 当旗舰——旗舰是 `gpt-5.5`，`gpt-5.4-mini` 是它的轻量款。
...
`config.toml` 里那一大堆旋钮（模型、沙箱、审批、推理强度、服务层级、MCP……）的完整讲解在〔[18 config.toml 配置详解](/blog/codex/18-config/) 〕，这里你只需建立一个认知：**别想着「翻译」整个 `settings.json` ，而是照着 [18] 那张键表，把你真正在用的那几条重新配一遍。** 大多数人实际改的也就模型、沙箱、审批这三五行。
...
权限这块的完整玩法（三档怎么组合、rules 怎么写、`--yolo` 红线在哪）在〔[15 权限、沙箱与审批](/blog/codex/15-permissions/) 〕，迁移时重点是先把「两个独立旋钮」这个新心智建立起来，别拿「列白名单」的旧框架硬套。
...
我自己迁移过来后第一周的真实体感：**敲命令基本不用查**——`/model` 切模型、`/status` 看状态、`/compact` 压上下文全是顺手就来。唯一卡过一下的是想调权限，习惯性按 `Shift+Tab` 没反应，才想起来 Codex 得敲 `/permissions` 。完整的斜杠命令速查在〔[12 斜杠命令与快捷键](/blog/codex/12-slash-commands/) 〕，但凭旧习惯先用着、卡了再查，基本够用。
...
下一篇〔[33 Windows 使用要点](/blog/codex/33-windows/) 〕——前面好几处动手我都让 Windows 用户「在 Git Bash 或 WSL 里敲」，那到底为什么？Windows 上跑 Codex，沙箱、路径、终端这些地方有哪些专属的坑？留个小思考：本篇讲的沙箱机制，在一个原生没有 Linux 那套隔离能力的系统上，Codex 又是靠什么把缰绳攥住的？下一篇就把 Windows 这条线单独讲透。

---
title: "15 · 权限、沙箱与审批：放多松、收多紧，自己拧"
description: "说个我去年冬天干的蠢事。那会儿我刚把 Codex 配熟…"
---

> 📚 **系列导航**：上一篇 [14 · 常见工作流](/blog/codex/14-workflows/) 把 Codex 揉进了你的日常开发节奏。这一篇换个维度——管的不是「它怎么干活」，而是「它敢动多大」：从一行命令都要问你，到全自动放飞，**沙箱和审批这两个旋钮到底怎么拧、什么场景拧到哪一档，这一篇给你拧明白**。
...
![权限的两个旋钮：沙箱档 × 审批档](https://img.yuancong.ai/guide/codex/15-two-dimensions@2x.png)
...
![沙箱与审批的两层决策：圈内直接做 / 出圈再看审批三档（untrusted 最严 · on-request 默认 · never 最松）](https://img.yuancong.ai/guide/codex/02-approval-flow@2x.png)

这张图在说：**每要做一步，Codex 先问「在不在沙箱圈内」（沙箱说了算），出圈了再看审批三档——`untrusted` 最严、`on-request` 默认、`never` 最松——决定要不要停下来问你（审批说了算）**。两道关卡顺着走，缺一不可。注意图左下那行小字：`untrusted` 是个例外，它连**圈内的非只读操作**也会先问你，所以审批并不只在「出圈」时才发生。

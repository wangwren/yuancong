---
title: "08 · 命令行 CLI 上手"
description: "都说「图形界面对小白最友好，能不碰命令行就别碰」——但说句实话，这话用在 Codex 身上，得打个对折。"
---

> 📚 **系列导航**：上一篇 [07 桌面 App 全景](/blog/codex/07-desktop-app/) 带你把图形界面那套点法摸熟了——看 diff、并排跑任务、可视化审批。这一篇调头钻进终端，搞懂 `codex` 这条命令到底怎么用：命令结构怎么拆、交互界面长啥样、常用选项和斜杠命令各管什么。下一篇 [09 IDE 扩展（VS Code 等）](/blog/codex/09-ide/) 再把它塞进编辑器里。
...
![Codex CLI 的 TUI 界面：对话区、输入框、状态栏三块](https://img.yuancong.ai/guide/codex/08-tui-three-zones.png)
...
![token-tracker 显示用量：项目名、token 剩余、上下文占比、当前模型](https://img.yuancong.ai/guide/codex/08-tui-output.png)

---
title: "25 · Worktrees 并行隔离：让几个 Codex 各干各的，互不打架"
description: "说个我今年三月干的蠢事。"
---

> 📚 **系列导航**：上一篇〔[24 规则与钩子（Hooks）](/blog/codex/24-hooks/)〕给 Codex 装上了「闸门」和「扳机」，让重复的破事自动化。这一篇换个维度——**不是让一个 Codex 多干一件事，而是让好几个任务真正同时开工、还互不踩脚**。Worktree（工作树）就是 Codex 用来做这件事的核心隔离手段：怎么建、怎么用、为什么同一个分支不能同时在两处检出、Handoff（交接）又是怎么把活儿在前台后台之间搬来搬去。下一篇〔[26 Git 与 GitHub 集成](/blog/codex/26-git-github/)〕再讲改完之后怎么提交、推送、开 PR。
...
第 07 篇讲桌面 App 时我已经把 Worktree **提过一下**——它是新线程时三个模式（Local / Worktree / Cloud）里的一个，配 Handoff 在前后台之间搬（其中 Cloud（云端）模式本篇不展开，单独放在〔[10 云端任务](/blog/codex/10-cloud/)〕讲）。这一篇咱们往下挖：**它底层到底怎么隔离的、为什么有「分支只能在一处检出」这条铁规、Handoff 两个方向怎么走、副本攒多了怎么清。** 这些坑，不讲清楚迟早自己踩。
...
![Worktrees：多任务并行不打架](https://img.yuancong.ai/guide/codex/25-worktree-parallel@2x.png)
...
下一篇 **〔[26 Git 与 GitHub 集成](/blog/codex/26-git-github/)〕**——这一篇你已经撞见好几个 Git 动作了：worktree 转分支、Handoff 背后那些被 Codex 悄悄替你处理的 git 操作、改完之后要提交推送……但这些大多还停在「Codex 替你做」的层面。下一篇就把 Codex 的 Git / GitHub 能力正面摊开：在 App 里直接 commit、push、开 PR，diff 面板怎么逐块暂存或还原，再到怎么跟 GitHub 联动。想想看：**worktree 把活隔离好了，最后总得有一步把它干净地并回主线——那一步，正是下一篇的主场。**

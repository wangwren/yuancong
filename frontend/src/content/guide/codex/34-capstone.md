---
title: "34 · 综合实战：从零给一个 TODO 小工具加功能、提交一次"
description: "兄弟们，今天不上课，咱们一起从零做一个小东西。"
---

> 📚 **系列导航**：上一篇〔[33 Windows 使用要点](/blog/codex/33-windows/) 〕把 Windows 上那些容易卡住人的坑——路径、换行符、终端、沙箱差异——一条条理顺了。这一篇咱们不讲新功能，干一件更过瘾的事：**把前面三十几篇学的零件，亲手装成一台能跑的整机**。下一篇〔[35 命令与配置速查表](/blog/codex/35-cheatsheet/) 〕再把全篇用到的命令、配置键一张表收口，当工具书随时翻。
...
Codex 一进项目是张白纸——它不知道这是什么项目、用什么跑、有什么规矩。**开工前的第一件事，是给它一份「交接清单」。** 这正是〔[11 项目说明书 AGENTS.md](/blog/codex/11-agents-md/) 〕讲的事。
...
须知贴好了，该派活了。**派活的质量，直接决定它干出来的活儿有多准。** 这是〔[13 提示词写法](/blog/codex/13-prompting/) 〕的核心：一句信息量约等于零的「加个删除功能」，它只能脑补；把「四件套」说全，它才不跑偏。
...
派活之前（或会话刚起时），得先想清楚：**这个活儿要让 Codex 动多大？** 这是〔[15 权限、沙箱与审批](/blog/codex/15-permissions/) 〕的事——沙箱管「它能碰到哪」，审批管「动手前问不问你」。
...
- **活儿「又多又能并行」→ 叫子代理。** 比如不是删一个功能，而是要把二十个文件里的过期写法批量改掉。这时让主代理拆活、多个子代理并行去改，比一个人顺序磨快得多。怎么拆、怎么派，看〔[21 子代理](/blog/codex/21-subagents/) 〕——记住它的脾气：**只有你开口它才拆，不会自作主张。**
- **活儿「要够外部世界」→ 接 MCP。** 比如删除前要先查公司内部某个系统「这条待办是否已归档」，这种 Codex 自己够不着的外部能力，就靠 MCP 这个「外接口」接进来，见〔[20 用 MCP 接外部工具](/blog/codex/20-mcp/) 〕。
...
功能做对、测试全绿，临门一脚是把这次改动**干净地提交**。这是〔[26 Git 与 GitHub 集成](/blog/codex/26-git-github/) 〕的本事，也是整条链路的收口：**审查和提交信息它来拟，按下提交那一下你来定。**
...
| ② 写须知 | `AGENTS.md` 交代背景、怎么跑、什么规矩 | 〔[11](/blog/codex/11-agents-md/) 〕 |
| ③ 派活 | 目标 + 范围 + 约束 + 验收，四件套说全 | 〔[13](/blog/codex/13-prompting/) 〕 |
| ④ 设权限 | `workspace-write` + `on-request` 划好圈 | 〔[15](/blog/codex/15-permissions/) 〕 |
| ⑤ 升级（按需） | 又多又并行叫子代理、够不着外部接 MCP | 〔[21](/blog/codex/21-subagents/) 〕〔[20](/blog/codex/20-mcp/) 〕 |
| ⑥ 自验证 | 让它跑 `unittest` 跑到全绿再交付 | 〔[13](/blog/codex/13-prompting/) 〕 |
| ⑦ 提交 | Codex 拟 diff 与信息，你确认后 `commit` | 〔[26](/blog/codex/26-git-github/) 〕 |

**这条链的顺序不是我拍脑袋排的，是真实开发的自然节奏**：先让它懂项目（须知），再说清要什么（派活），再框住它能动多大（权限），干完自己验（测试），最后定稿入库（提交）。

模型选什么、推理强度配多大，这条链路里我用的就是默认那套——**旗舰 `gpt-5.5` + 默认推理强度（一般是 `medium`）**，这个 TODO 小活儿完全够用，没必要拉到 `high`。要是你碰上的是「跨模块大重构」那种硬骨头，再回〔[30 怎么选模型](/blog/codex/30-models/) 〕按场景调档。**这一篇我刻意全程用默认配置走，就是想让你看清：链路本身才是主角，调参是锦上添花。**
...
下一篇〔[35 命令与配置速查表](/blog/codex/35-cheatsheet/) 〕，咱们把整个 Codex 篇用到的命令、配置键、常用斜杠命令收进一张能随时翻的速查表——你刚跟着做完这一遍，正好趁热想一个问题：**这条链路里，哪几个命令是你下次盲敲都不会错的，哪几个还得回头查？** 那张表，就是为「还得查」的那部分准备的。

---
title: "03 · 安装与登录（Mac / Windows / Linux）"
description: "OpenAI 在 2026 年初把 Codex 拆成了四个入口：网页版、桌面 App、CLI、IDE 扩展。我自己从 CLI 一路用到桌面 App…"
---

> 📚 **系列导航**：上一篇 [02 · 核心概念速览](/blog/codex/02-core-concepts/) 把 Codex 的几个关键词（代理、沙箱、审批、本地 / 云端）讲明白了。这一篇带你真正把它装到自己电脑上——**桌面 App 和 CLI 两条线都覆盖**，登录授权、三平台差异、常见装机故障一次说透。下一篇 [04 · 订阅与计费](/blog/codex/04-pricing/) 再聊钱的事。
...
账号和计费的细节，下一篇 [04 · 订阅与计费](/blog/codex/04-pricing/) 专门展开。本篇默认你手里有一个能用的 ChatGPT 套餐或 API key。
...
![Codex 桌面 App 主界面：左侧导航、中间任务输入、连接卡片](https://img.yuancong.ai/guide/codex/03-app-ui.png)
...
![两条安装线：桌面 App 与 CLI](https://img.yuancong.ai/guide/codex/03-install-flow@2x.png)
...
![Windows 装 Codex 的三条路：WSL2 / 原生 elevated 沙箱 / 原生 unelevated 沙箱](https://img.yuancong.ai/guide/codex/03-windows-decision@2x.png)
...
![第一次跑通 Codex 的完整循环：mkdir → codex 登录 → 大白话指令 → 审批 → Git 检查点](https://img.yuancong.ai/guide/codex/03-first-loop@2x.png)
...
下一篇 **[04 · 订阅与计费](/blog/codex/04-pricing/)**——把钱的事说透：ChatGPT 各档套餐分别给多少 Codex 额度？API key 按量付费到底怎么算？跟 Claude Code 比哪个更划算？刚装好正想撒欢用，先搞清楚自己手里的账号每天能用多少，免得跑到一半被限额拦下来。

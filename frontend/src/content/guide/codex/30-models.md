---
title: "30 · 怎么选模型：同一句话，到底该派哪个模型去跑"
description: "都说「选最强的模型准没错」，我信了挺久。"
---

> 📚 **系列导航**：上一篇〔[29 Slack / Linear 与 SDK 集成](/blog/codex/29-integrations/)〕讲的是「在别处召唤 Codex、把它嵌进自己产品」的进阶玩法。这一篇把镜头拉回本地最日常的一个决定——**你敲下回车那一刻，背后到底是哪个模型、用多大力气在帮你干活**。下一篇〔[31 进阶技巧与提速](/blog/codex/31-speed/)〕再讲怎么让整条流程更快更省。
...
![模型档位怎么挑](https://img.yuancong.ai/guide/codex/30-model-tiers@2x.png)
...
先说清一件事：**推理强度的可选档位随模型变化，不是固定一套**（仅对支持推理的模型、Responses API 生效）。下面是常见的几档，从低到高：

| 档位 | 含义 | 适合 |
| --- | --- | --- |
| `none` | 完全不做额外推理（部分模型支持） | 纯指令执行 |
...
注意两点：**`medium` 一般是默认起点**（具体默认值以你本地版本为准）；**这张表不是每个模型都全支持**——比如 `none` 和 `xhigh` 都只有部分模型吃。当前模型到底能选哪几档，直接在 `/model` 面板里看，选了无效或报错面板通常会有提示；拿不准就查它的官方说明页。
...
**④ 给子代理单独配（分工干活）。** 主代理用旗舰统筹，把简单的批量活儿派给挂着 `gpt-5.4-mini` 的子代理——这是性价比最高的打法，细节见〔[21 子代理（Subagents）](/blog/codex/21-subagents/) 〕。
...
下一篇〔[31 进阶技巧与提速](/blog/codex/31-speed/) 〕，咱们从「派谁、用多大力」往前再走一步：聊聊那些能让整条工作流更快、更省、更少返工的进阶技巧。留个小思考——你有没有发现，**很多时候拖慢你的不是模型不够强，而是你给的上下文太乱、让它走了冤枉路**？这恰恰是下一篇要解的题。

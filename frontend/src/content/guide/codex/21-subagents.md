---
title: "21 · 子代理（Subagents）：把活儿拆出去并行跑，但只有「你开口」它才拆"
description: "兄弟们，今天聊 Codex 里最唬人、也最容易用歪的一个功能——子代理。"
---

> 📚 **系列导航**：上一篇 [20 · 用 MCP 接外部工具](/blog/codex/20-mcp/) 教你给 Codex 接上外部工具，让它能查文档、连服务。这一篇换个思路——不是给它「加工具」，而是教你把活儿**拆出去并行跑**：子代理（Subagent），一批带独立模型、独立指令、独立权限的专项助手，几个同时干，干完把结论汇总成一份交回来。
...
- Codex 自带的三种内置 agent（`default` / `worker` / `explorer`）——是「类型」不是「实例上限」，开箱即用、不用配
...
![子代理：并行分发、各自上下文、只回摘要](https://img.yuancong.ai/guide/codex/21-subagent-isolation@2x.png)
...
![子代理何时拆并行：小改 / 强依赖留主线；任务大且几块独立时拆出 N 个 agent 并行跑、汇总成结论](https://img.yuancong.ai/guide/codex/21-subagent-flow@2x.png)

这张图在干什么：先判断「该不该拆」，不该拆就留主线；该拆且你开了口，Codex 才派出几路 agent 并行，等齐所有结果后汇总成一份交回主线——**注意那个「你开口」的岔口，没它 Codex 不会自己拆。**

---

## 03 不用配也能拆：三种内置 agent + 怎么唤起
...
> 💡 一句话总结：三种内置 agent（`default` / `worker` / `explorer`，是类型不是实例上限）开箱即用；但 Codex **绝不自动拆**，得你在话里明说「开几个 agent 并行、等齐、回汇总」才动——它会等所有结果齐了再给你一份汇总。
...
内置这三种 agent 能应付不少场景，但你要是发现自己老在重复同一类活——每次审查都强调「像 owner 那样审、盯正确性和安全」——那就该把这套固化成一个**自定义 agent**，下次一句话唤起。
...
| 内置 / 自定义 | 内置三种开箱即用；自定义写 TOML | 必填 `name` / `description` / `developer_instructions` |
| 怎么选模型 | `model` + `model_reasoning_effort` | 侦察用快脑、审查用强脑 |

**你现在应该能：** 判断一个任务该不该拆给子代理（而不是一激动就拆）；用好内置三种 agent，或往 `~/.codex/agents/` 写一个带专属指令、专属模型、锁定权限的自定义 agent；并且记牢——**得你开口它才拆，干完它只把摘要汇总回来。** 这套「该并行就并行、该自己干就自己干」的分寸感，才是子代理真正的门槛——功能十分钟学会，分寸得用出来。

...
description: "先放一段真事改的对话，你大概就懂这篇在讲啥了。"
---

> 📚 **系列导航**：上一篇〔[28 非交互模式 codex exec](/blog/codex/28-noninteractive/) 〕讲的是「一句话丢进去、跑完吐结果就退」的无人值守跑法——那是把 Codex 接进脚本和 CI 的第一块拼图。这一篇再往外走两步：**一是不打开终端，在 Slack、Linear 里直接喊一声就让 Codex 干活；二是用官方 SDK / App Server，把 Codex 当成一个零件嵌进你自己写的程序和产品里。** 下一篇〔[30 怎么选模型](/blog/codex/30-models/) 〕回到本地，专门讲「同样一句话，到底该派哪个模型去跑」。
...
![Slack @Codex 完整流程：@提需求 → 回任务链接 → 云端建容器拉仓库 → Agent 干活 → 回 thread → 点链接进 Web](https://img.yuancong.ai/guide/codex/29-slack-flow@2x.png)
...
下一篇 **〔[30 怎么选模型](/blog/codex/30-models/) 〕**——这一篇你不管是在 Slack 里 @Codex、还是用 SDK 写程序，有个绕不开的问题一直没细讲：**到底该让哪个模型去干这活？** 你大概注意到了，SDK 示例里要填 `model=...`、CLI 里有 `--model`、`/model` 能当场切——但「这活该派强模型还是快模型、推理强度调多高」，一直没说清。下一篇就专门掰开：**Codex 背后这几个模型各有什么脾气、什么活该用哪个、怎么在「又快又省」和「又强又准」之间拿捏。** 先想想：你最近让 Codex 干的活里，有哪几件其实用个更快更便宜的模型就够了？

---
title: "24 · 规则与钩子（Rules & Hooks）：给 Codex 装上「卡点」和「扳机」"
description: "先还原一段我上个月真实发生的对话。我跟同事在排查一个 CI 老挂的问题："
---

> 📚 **系列导航**：上一篇〔[23 插件（Plugins）](/blog/codex/23-plugins/)〕教你把别人打包好的能力一键装进 Codex。这一篇聊两件更底层、也更「你说了算」的事——**规则（Rules）管「哪些命令能在沙箱外跑」，钩子（Hooks）管「在干活流程的某个固定点自动替你跑一段脚本」**。一个是闸门，一个是扳机，配好了，很多「每次都得盯着、每次都得手敲」的破事就再也不用你操心了。下一篇〔[25 Worktrees 并行隔离](/blog/codex/25-worktrees/)〕再讲怎么让多个 Codex 各干各的、互不打架。
...
那它们和你已经会写的 AGENTS.md（项目说明书，第 [11](/blog/codex/11-agents-md/) 篇讲过）差在哪？差在一个**根本点**：
...
先说规则解决的痛点。第 [15](/blog/codex/15-permissions/) 篇讲过，沙箱是「按圈划」的**粗**边界——圈里随便动、出圈就问。但有时候你想要的是**按命令**的精细控制：「`gh pr view` 哪怕出圈也直接放行，别烦我」「`grep` 一律禁掉，逼我自己用 `rg`」。这种需求，扩大整个沙箱太粗暴，写一条规则才对症。
...
这条特别重要，第 [15](/blog/codex/15-permissions/) 篇点过，这里展开。碰到 `git add . && rm -rf /` 这种**把多条命令塞进一行**的，Codex 在安全的前提下会用 tree-sitter **把它拆成单条逐个判**，再取「最严的结果」：
...
回想第 [02](/blog/codex/02-core-concepts/) 篇讲的「代理循环」——Codex 干活是「想 → 做 → 看」转圈。这些事件，正好散布在这个循环的前前后后。先画张图，看几个最常用的事件卡在哪儿：

![Codex 钩子生命周期：SessionStart → UserPromptSubmit → 要不要用工具？要用走 Pre/Post 循环；不用 Stop 收尾](https://img.yuancong.ai/guide/codex/24-hook-lifecycle@2x.png)
...
![钩子挂在生命周期固定点](https://img.yuancong.ai/guide/codex/24-lifecycle-hooks@2x.png)
...
逻辑和第 [15](/blog/codex/15-permissions/) 篇讲配置时一样：**全队都该有的钩子（比如「改完一律格式化」）写进项目的 `.codex/` 提交进 git；只是你自己想要的写进 `~/.codex/`**。两个细节记一下：
...
> ⚠️ 回到安全主线（第 [16](/blog/codex/16-security/) 篇）：**加任何钩子前先在 `/hooks` 里把它的命令逐行审一遍**，尤其别从来路不明的插件或仓库里抄整段钩子就信任。信任机制是 Codex 替你设的最后一道防线，但点「信任」那一下的判断，还是你自己的。
...
这一节是**看懂一切的钥匙**。前面的脚本怎么拿到命令内容？钩子怎么「拦」住一条命令？答案全在这套「对话机制」里，和第 [16](/blog/codex/16-security/) 篇讲的输入输出是一脉相承的。
...
练完拆掉很简单：如果这个练习目录里只有这一条钩子，直接删掉 `.codex/hooks.json`，再用 `rm ~/codex-bash-log.txt` 删掉日志文件。若你还有别的钩子，就不要删整个文件，只删这条练习钩子对应的 `PostToolUse` / `matcher: "Bash"` 配置块，并确保剩下的 `hooks.json` 仍是合法 JSON。不要把文件内容清空；空文件会被当成非法 JSON 解析，至少要保留 `{ "hooks": {} }` 这种合法空配置。
...
下一篇 **〔[25 Worktrees 并行隔离](/blog/codex/25-worktrees/)〕**——这一路你都是「一个 Codex 干一件事」。但真到了开发节奏快起来的时候，你会想：**能不能让一个 Codex 修 bug、另一个同时做新功能，俩互不踩脚？** 直接开两个会话改同一个仓库，分分钟把彼此的改动覆盖掉。下一篇聊 Git worktree 这个老牌但极好用的招——给每条任务一个独立工作区，让多个 Codex 真正并行、各扫门前雪。想想看：你现在要同时推进两件事，是不是只能干完一件再开下一件？

---
title: "17 · 电脑操控与浏览器（Computer Use）：让 Codex 长出手"
description: "OpenAI 在官方文档里给 Computer Use 圈了一条很硬的地理红线：上线时它在 macOS 和 Windows 可用，但欧洲经济区（EEA）、英国…"
---

> 📚 **系列导航**：上一篇 [16 · 安全与风险边界](/blog/codex/16-security/) 把「Codex 能闯多大祸、边界在哪」这条线画清楚了。这一篇给它再开一扇门——**让 Codex 直接看你的屏幕、点你的桌面应用、开你的浏览器**：测一个原生 App、复现一个只在界面上才出现的 bug、在你登录好的网站里替你点几下。前面所有篇章里 Codex 干的活都在「文件 + 命令行」那一亩三分地，这篇是它头一回伸手碰图形界面。下一篇 [18 · config.toml 配置详解](/blog/codex/18-config/) 再回到配置文件，把那一摞旋钮一次拧明白。
...
这一篇就讲清三件事：Codex 碰图形界面的**三条路**（内置浏览器、Chrome 扩展、电脑操控）各管哪一块、怎么开起来、以及操作你「真桌面 / 真浏览器」时的安全边界在哪。
...
![Codex 电脑操控三条路：公开页用浏览器 / 登录态用 Chrome 扩展 / 桌面 App 才用电脑操控](https://img.yuancong.ai/guide/codex/17-ui-routes@2x.png)
